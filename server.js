...
// --- 4. Credentials (kept inline for now; consider env vars in production) ---
const paragonApiConfig = {
  clientId: '8fbF4ONttMVXbsp2WKCK',
  clientSecret: 'dH8o7fxwLISCrMmZ14Sj2knt6EM6ewAOcvM2oZvd',
  tokenUrl: 'https://paragonapi.com/token', // confirm URL with your provider
  apiUrl: 'https://api.paragonapi.com/api/v2/OData/bk9',
};

const GEMINI_API_KEY =
  'AIzaSyACKfnIE47Ig4PZyzjygfV9VZxUKK0NPI0';
...
app.get('/api/comps', async (req, res) => {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      throw new Error(`Auth failed with MLS API (${tokenResponse.status})`);
    }
    ...
    if (!propertyResponse.ok) {
      throw new Error(`Failed to fetch property data (${propertyResponse.status})`);
    }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res
        ...
        .json({ message: 'Error from Gemini API.', details: errorBody });
    }
