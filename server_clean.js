...
// --- 5.7 Debug Open House Times Endpoint ---
app.get("/api/debug-open-house-times", async (req, res) => {
  try {
    // Limit to 10 results for debug
    const limit = 10;
    const selectFields = [
      "ListingId",
      "UnparsedAddress",
    ...
    const odata = `$select=${selectFields}&$top=${limit}`;
    ...
    // Return only open house time fields for inspection
    const debugData = rows.map(r => ({
      ListingId: r.ListingId,
      Address: r.UnparsedAddress,
      City: r.City,
      State: r.StateOrProvince,
      Zip: r.PostalCode,
      OpenHouseDate: r.OpenHouseDate,
      OpenHouseStartTime: r.OpenHouseStartTime,
      OpenHouseEndTime: r.OpenHouseEndTime,
      OpenHouseDescription: r.OpenHouseDescription,
    ...
    console.error("Debug open house times error:", error);
...
// --------------------------------------------------------------
// Helper function to process open house data
// --------------------------------------------------------------
function processOpenHouseData(property) {
  ...
  // Check for direct open house fields
  if (property.OpenHouseDate) {
    try {
      // Parse date to standard ISO format
      const date = new Date(property.OpenHouseDate);
      if (!isNaN(date.getTime())) {
        result.openHouseDate = date.toISOString().split('T')[0]; // YYYY-MM-DD
        result.hasOpenHouse = true;
        
        // Get day of the week
        const dayNames = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
        result.openHouseDays = [dayNames[date.getDay()]];
      }
    } catch (e) {
      // Invalid date, ignore
    }
    ...
    if (property.OpenHouseStartTime) {
      result.openHouseStartTime = formatTimeString(property.OpenHouseStartTime);
    }
    
    if (property.OpenHouseEndTime) {
      result.openHouseEndTime = formatTimeString(property.OpenHouseEndTime);
    }
    
    // Default status for explicitly scheduled open houses
    result.openHouseStatus = "scheduled";
  }
  
  // If no direct open house data, try to extract from remarks
  if (!result.hasOpenHouse && (property.PublicRemarks || property.RemarksPublic)) {
    const remarks = property.PublicRemarks || property.RemarksPublic || "";
    const openHouseInfo = extractOpenHouseFromRemarks(remarks);
    
    if (openHouseInfo) {
      result.hasOpenHouse = true;
      Object.assign(result, openHouseInfo);
    }
  }
  
  return result.hasOpenHouse ? result : null;
}

// Helper to format time strings to 24hr format
function formatTimeString(timeStr) {
  if (!timeStr) return null;
  
  // Handle various formats
  try {
    // Try to handle common formats like "2:00 PM" or "14:00"
    if (timeStr.includes("PM") || timeStr.includes("AM")) {
      ...
      // Assume it's already in 24hr format or handle other formats
...
// Helper to extract open house information from remarks
function extractOpenHouseFromRemarks(remarks) {
  ...
  // Array of day names to look for
  const dayNames = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
  const dayAbbreviations = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  
  // Check for days mentioned
  dayNames.forEach((day, index) => {
    const dayRegex = new RegExp(`\\b${day}\\b|\\b${dayAbbreviations[index]}\\b`, 'i');
    if (dayRegex.test(remarks)) {
      result.openHouseDays.push(day);
    }
  });
  
  // Look for date patterns: MM/DD, MM-DD, Month DD
  const dateRegex = /\b(0?[1-9]|1[0-2])[\/\-](0?[1-9]|[12][0-9]|3[01])\b|\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* (0?[1-9]|[12][0-9]|3[01])\b/gi;
  const dateMatches = remarks.match(dateRegex);
  ...
  if (dateMatches && dateMatches.length > 0) {
    ...
    try {
      // Add current year for proper parsing
      ...
      if (!isNaN(date.getTime())) {
        result.openHouseDate = date.toISOString().split('T')[0];
      }
    } catch (e) {
      // Invalid date, ignore
    }
  ...
  // Look for time patterns: HH:MM-HH:MM, H-H PM, H:MM AM - H:MM PM
  const timeRegex = /\b((1[0-2]|0?[1-9]):?([0-5][0-9])?\s*(am|pm)?\s*[-–—to]\s*(1[0-2]|0?[1-9]):?([0-5][0-9])?\s*(am|pm)?)\b/i;
  const timeMatch = remarks.match(timeRegex);
  ...
  if (timeMatch && timeMatch[0]) {
    ...
    if (parts.length === 2) {
      ...
      // Try to parse the times to standardized format
      try {
        ...
        if (startTime.match(/\d+\s*(am|pm)/i)) {
        ...
        if (endTime.match(/\d+\s*(am|pm)/i)) {
          const isPM = endTime.toLowerCase().includes("pm");
          const hourMatch = endTime.match(/\d+/);
          if (hourMatch) {
            let hour = parseInt(hourMatch[0]);
            if (isPM && hour < 12) hour += 12;
            if (!isPM && hour === 12) hour = 0;
        ...
        result.openHouseStartTime = startTime;
        result.openHouseEndTime = endTime;
      } catch (e) {
        // Time parsing failed, use raw strings
        result.openHouseStartTime = startTimeStr;
        result.openHouseEndTime = endTimeStr;
      }
    }
  }
  
  // Check if we found enough information
  const hasTimeInfo = result.openHouseStartTime && result.openHouseEndTime;
  const hasDateInfo = result.openHouseDate || result.openHouseDays.length > 0;
  
  return (hasTimeInfo || hasDateInfo) ? result : null;
}
...
app.get("/api/property-search-advanced", async (req, res) => {
  ...
  try {
    ...
    const selectFields = [
      ...
      "WaterfrontYN",
      "NewConstructionYN",
      "OpenHouse", // Array of open house events
      "OpenHouseDate",
      "OpenHouseStartTime",
      "OpenHouseEndTime",
      "OpenHouseDescription",
    ...
    const enriched = rows.map((r) => {
      ...
      return {
        ...
        openHouse: processOpenHouseData(r),
      };
    });
